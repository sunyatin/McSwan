...
#' @title Perform genome scan on simulated genomic fragments
#' @description This function is equivalent to \code{thin(gscan(...))} except that it works for simulated genomic fragments stored in an \code{validationTable} object.
#' @param valtb a \code{validationTable} object generated by \code{\link{generate_pseudoobs}}
#' @param reftb an initialized \code{referenceTable} object with non-empty \code{PRIORS} slot
...
#' @param ... other arguments passed to \code{\link{thin}}, please consult \code{?thin}
#' @return A \code{validationTable} object with an additional \code{ANALYSIS} slot containing the results of the genome scans over the simulated fragments.
#' @examples Please refer to the vignette.
#' @export
ABC_sliding_validation <- function(valtb, 
                               reftb,
                               minSNP = 10, 
                               cutoff = 10, 
                               tolABC = .01, 
...
#' @examples Please refer to the vignette.
ABC_summary.validationTable <- function(X, file) {
