...
#' @keywords internal
ABC_impute <- function(postproba, cutoff) {
...
ABC_analyze <- function(target, 
  ...
  doESTIMATION <- TRUE
  #plot_simCloud <- FALSE
if (do_1dsfs) target = convert_to_1dSFS(target)
  # target conditions
...
#' @export
ABC_gscan <- function(X, reftb, minSNP, startPos = NULL, lastPos = NULL, windowSlide = NULL, tolABC = .01, tolGFIT = .05, cutoff = 5, printProgress = T, plot_simCloud = FALSE) {
