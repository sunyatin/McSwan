...
#' @param tempDir path/name of a temporary directory (will be recursively created if it does not exist; if the directory already exists, its content will be overridden)
#' @param pythonPath path to the \code{python} executable, simply give "python" if python is callable as an environment variable
#' @param javaPath path to the \code{java} executable, simply give "java" if python is callable as an environment variable
...
set_session <- function(tempDir, pythonPath = "python", javaPath = "java") {
  ...
  if (!file.exists(tempDir)) {
    ...
    cat("Please note that the temporary directory \"",normalizePath(tempDir),"\" already exists. Data will be overridden.\n", sep="")
  }
...
#' @title Generates log10-uniform distributions
#' @description This function samples values out of a log10-uniform distribution.
...
#' @title Calculates the mode of a univariate distribution
#' @param x a vector of numeric values
#' @return A numeric value corresponding to the mode of the empirical distribution.
#' @export
...
#' @param X a \code{referenceTable} object (generated with \code{\link{generate_priors}})
...
#' @param X a \code{validationTable} object (generated with \code{\link{generate_pseudoobs}})
...
#' @title Print a summary of an \code{observedDataset} object content
#' @param X a \code{observedDataset} object (generated with \code{\link{get_SFS}})
...
#' @description Given a window length, computes the distribution of within-window SNP counts in the observed dataset. This function is useful to get an idea about the SNP density in the VCF to suggest appropriate window size ranges.
#' @param obs an \code{observedDataset} object, generated with \code{\link{get_SFS}}
#' @param windowSize (integer) a window size (in base pairs)
#' @return A summary of the distribution of the within-window SNP counts.
#' @seealso \code{\link{generate_priors}}
